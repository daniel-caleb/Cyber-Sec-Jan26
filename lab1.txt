...
This lab is designed to help you become comfortable with:
...
* Use commands like `cd`, `ls`, and `pwd` to navigate directories
* Create and remove files/directories using `touch`, `mkdir`, `rm`
* Copy, move, and rename files using `cp` and `mv`
* View and edit files using `cat`, `less`, `nano`, or `vim`
...
* Add at least 3 Linux commands to `notes.txt`

* Add the following to `backup.sh`:
...
* Move `todo.txt` from `documents/` to `projects/`
...
*(Hint: Use `>` or `>>` for output redirection)*
...
3. What command would you use to remove a directory with contents?
